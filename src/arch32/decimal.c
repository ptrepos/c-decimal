...
static inline uint32_t __mg_deciaml_set_bits(uint32_t dest, uint32_t value, uint32_t mask, int bitindex)
{
...
static inline void __mg_decimal_get_fraction(const mg_decimal *value, mg_uint256 *buf)
{
	...
	buf->word[3] = (uint32_t)__mg_decimal_get_bits(value->w[3], FRACTION_MASK, FRACTION_BITINDEX);
	buf->word[4] = 0;
	buf->word[5] = 0;
	buf->word[6] = 0;
	buf->word[7] = 0;
...
MG_DECIMAL_API mg_decimal_error mg_decimal_to_string(const mg_decimal *value, /*out*/char *buf, int bufSize, /*out*/int *requireBufSize)
{
	...
	while(!mg_uint256_is_zero(fraction)) {
		...
		err = mg_uint256_div(fraction, v10e9, tmp);
		if(err != 0)
			return err;

		subfraction = (uint32_t)mg_uint256_get_int64(fraction);
