...
MG_DECIMAL_API int mg_decimal_compare(const mg_decimal *op1, const mg_decimal *op2)
{
	...
	if(scale1 == scale2)
		...
		return mg_uint256_compare(fraction1, tmp) * flag;
	} else {
		mg_uint256_mul128(fraction1, mg_uint256_get_10eN(scale1 - scale2), tmp);
		return mg_uint256_compare(tmp, fraction2) * flag;
	}
}
...
MG_DECIMAL_API mg_decimal_error mg_decimal_multiply(const mg_decimal *op1, const mg_decimal *op2, /*out*/mg_decimal *ret)
{
	...
	if (_decimal_is_overflow(fraction)) {
		int rounded_scale;
		err = _truncate_max_digits(fraction, scale, /*out*/&rounded_scale);
		if(err != 0)
			goto _ERROR;
		scale = rounded_scale;
	...
	_decimal_set2(ret, sign, scale, fraction);
...
MG_DECIMAL_API mg_decimal_error mg_decimal_divide(const mg_decimal *op1, const mg_decimal *op2, /*out*/mg_decimal *ret)
{
	...
	if(scale > 0) {
		...
		scale = 0;
		if(_decimal_is_overflow(tmp)) {
			err = MG_DECIMAL_ERROR_OVERFLOW;
			goto _ERROR;
		}
		_decimal_set2(/*out*/ret, sign, scale, tmp);
	} else {
	...
	*quotient = *_q;
	...
	if(!mg_uint256_is_zero(fraction1)) {
		...
		do {
			...
			// q = q * 10^18
			overflow = mg_uint256_mul256x64(
					quotient, v10e18, /*out*/tmp);
			assert(overflow == 0);
			mg_uint256_swap(&quotient, &tmp);
