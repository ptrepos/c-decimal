...
static void convert_test2(const char *text1, const char *text2)
{
	...
	mg_assert(mg_decimal_parse_string(&value, text1) == 0);
	mg_assert(mg_decimal_to_string(&value, strbuf, 0, &size) != 0);
	mg_assert(mg_decimal_to_string(&value, strbuf, size, &size) == 0);
...
static void int_convert_test(int64_t value, const char *ret)
{
	char strbuf[1000];
	int size;
	mg_decimal value1;
...
void decimal_convert_test()
{
	...
	int_convert_test(-1000, "-1000");
	int_convert_test(-9999999999999, "-9999999999999");
	int_convert_test(-999999999999999999, "-999999999999999999");
	int_convert_test(-425415311, "-425415311");

	{
		mg_decimal value;
		mg_assert(mg_decimal_parse_string(&value, "1234567891234156498715634865156465151654152165453132416854114") == MG_DECIMAL_ERROR_OVERFLOW);
	}
